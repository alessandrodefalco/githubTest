...
//using namespace o2::base;
 ...
 {
   std::cout << "Print process settings of media.";
 ...
 {
   std::cout << "Print cut settings of media.";
 ...
 void MaterialManager::insertTGeoMedium(std::string modname, int localindex)
 {
   ...
   std::cout << "mapping " << modname << " " << localindex << " to " << mTGeoMediumMap[p]->GetName();
 }
 ...
 TGeoMedium* MaterialManager::getTGeoMedium(std::string const& modname, int localindex)
 {
   ...
   if (iter == mTGeoMediumMap.end()) {
     std::cout << "No medium registered for " << modname << " index " << localindex << "\n";
     return nullptr;
   }
   return iter->second;
 }
 ...
 void MaterialManager::loadCutsAndProcessesFromFile(const char* modname, const char* filename)
 {
   ...
   if (!cutfile.is_open()) {
     std::cout << "File " << filename << " does not exist; Cannot apply cuts";
     return;
   }
 ...
 void MaterialManager::printContainingMedia(std::string const& volumename)
 {
   ...
   if (vol == nullptr) {
     std::cout << "No volume found; Cannot query medias";
   }
